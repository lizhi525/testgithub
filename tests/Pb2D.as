package
{
	...
	public class Pb2D
	{
		...
		[Embed("pb2d.pbj", mimeType="application/octet-stream")]
		private var shaderClass:Class;
		private var shader : Shader=new Shader(new shaderClass() as ByteArray);;
		public function Pb2D(view:Stage3dView)
		{
			this.view = view;
			shader.data.src1.input = transformData;
			shader.data.src2.input = transformData2;
			shader.data.src3.input = transformData3;
		}
		...
		internal function enterFrameHandler():void
		{
			...
			var job:ShaderJob = new ShaderJob(shader, vertexData, numChild*4,1);
			job.start(true);
		...
		internal function add():void
		{
			...
			numChild++;
			shader.data.src1.width = shader.data.src2.width = shader.data.src3.width = numChild * 4;
			shader.data.src1.height = shader.data.src2.height = shader.data.src3.height = 1;
		}
	}
}
